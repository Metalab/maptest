...
-e 's/"world:\\\/\\\/chaos-zone\\\/\(.*\)"/"\\\/_\\\/global\\\/raw.githubusercontent.com\\\/jstsmthrgk\\\/2D-Welt-thueringen\\\/main\\\/\1"/g' \
$@
for f in $@
do
	tmp=$(mktemp)
	jq -ce '.properties += [{"name":"apiUrl","type":"string","value":"https://play.aaaaaaaaaaaadventu.re"}] | (.layers[] | select(.properties != null and .properties[].name=="jitsiRoom") | .properties) += [{"name":"jitsiUrl","type":"string","value":"meet.jit.si"}]' $f >$tmp
	mv $tmp $f
done
